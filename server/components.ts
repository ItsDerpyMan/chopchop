import { System, EntityData } from "./world.ts";
...
export class Position extends Component {
        ...
        return {
          ...
          update: (entities, world): boolean => {
                ...
                for (const [_, entity] of entities.entries()) {
                  if(entity.components.has(Position) && entity.components.has(Moving)){
                      const pos = entity.components.values().find((c: Component) => c instanceof Position) as Position;
                      const move = entity.components.values().find((c: Component) => c instanceof Moving) as Moving;
                      if (move.moving) {
                        pos.x += move.dx;
                        pos.y += move.dy;
                        move.moving = false;
    ...
    getPosition(entity: EntityData): undefined | Position{
        if(entity.components.size === 0)
            return undefined;
        if(!entity.components.has(Position))
            return undefined;
        return entity.components.values().find((c: Component) => c instanceof Position);
    }
}
